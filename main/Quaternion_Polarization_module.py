...
class Waveplate(np.quaternion):
    ...
    def __init__(self, phase_shift, axis_angle = 0, eigen_state = (0,0)):
        ...
        super().__init__(np.cos(self.phase_shift / 2),
                         np.cos(2*self.alpha + 2*self.axis_angle) * np.cos(2*self.chi) * np.sin(self.phase_shift / 2),
                         np.sin(2*self.alpha + 2*self.axis_angle) * np.cos(2*self.chi) * np.sin(self.phase_shift / 2),
